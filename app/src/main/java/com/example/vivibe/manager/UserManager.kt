...
class UserManager private constructor(private val context: Context){
    ...
    fun updatePremium(premium: Int) {
        try {
            val currentUser = _userState.value ?: return

            val updatedUser = currentUser.copy(premium = premium)

            val userData = JSONObject().apply {
                put("id", updatedUser.id)
                put("token", updatedUser.token)
                put("googleId", updatedUser.googleId)
                put("name", updatedUser.name)
                put("email", updatedUser.email)
                put("profilePictureUri", updatedUser.profilePictureUri)
                put("premium", updatedUser.premium)
            }
    ...
    fun getId(): String? = userState.value?.id

    fun isPremium(): Int = userState.value?.premium ?: 0
}